...
from UDL.hisr.HISR.SWAT_baselinev2.swat import *
...
class SWATnet(PatchMergeModule):
    def __init__(self, args):
        ...
        self.w = Block(img_size=self.img_size, patch_size=1, in_chans=34,
                 embed_dim=self.embed, depths=[4, 4, 4, 4], num_heads=[8, 8, 8, 8],
                 window_size=8, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True,
                 use_checkpoint=False, upscale=2, img_range=1., upsampler='', resi_connection='1conv')
            
            # img_size=self.img_size, in_chans=34, dd_in=3,
...
def build(args):
    ...
    model = SWATnet(args).cuda()
    num_params = 0
    for param in SWATnet(args).parameters():
        num_params += param.numel()
    print('[Network %s] Total number of parameters : %.3f M' % ('SWAT', num_params / 1e6))
