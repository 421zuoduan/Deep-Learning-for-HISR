...
class KernelAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        kernels = kernels.reshape(self.win_num, self.dim, self.kernel_size**2, self.dim).permute(1, 0, 2, 3).reshape(self.win_num, -1, self.dim)
...
class Window_Attention(nn.Module):
    ...
    def forward(self,H, W, x):
        ...
        # attention
        attn_windows_wa = self.attn(x_windows_wa)  # nW*B, window_size*window_size, C/2

        # merge windows
        attn_windows_wa = attn_windows_wa.view(-1, self.window_size, self.window_size, C//2)
        x = window_reverse(attn_windows_wa, self.window_size, H, W)  # B H' W' C/2
        x_wa = x.view(B, H * W, C//2)
