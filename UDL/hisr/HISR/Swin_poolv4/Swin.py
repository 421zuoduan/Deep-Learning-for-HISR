...
class WindowInterAttention(nn.Module):
    ...
    def __init__(self, input_resolution, dim, window_size, k_size, k_stride, k_padding, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        super().__init__()
        ...
        self.convlayer = ConvLayer(dim, k_size, k_stride, k_padding, self.nW, if_global=False)
        # self.wink_reweight = WinKernel_Reweight(dim, win_num=self.nW)
        self.fusion = nn.Conv2d(self.nW*self.dim, self.dim, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        # x:  B, nW*C, H, W
        x = x.repeat(1, self.nW, 1, 1)

        # x:  1, B*nW*C, H, W
        x = x.reshape(1, B*self.nW*C, H, W)

        # x:  1, B*nW*C, H, W
        x = self.convlayer(x, kernels, B)
        
        # x:  B, nW*C, H, W
        x = x.reshape(B, self.nW*C, H, W)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.window_inter_attn = WindowInterAttention(input_resolution, dim=dim, window_size=8, k_size=3, k_stride=1, k_padding=1, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop)
