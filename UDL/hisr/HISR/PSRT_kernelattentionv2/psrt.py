...
def ka_window_reverse(windows, window_size, H, W):
    ...
    """
    B = windows.shape[1]
    x = windows.view(H // window_size, W // window_size, B, -1, window_size, window_size)
...
class DynamicConv(nn.Module):
    def __init__(self, dim, kernel_size=3, stride=1, padding=1, dilation=1, groups=1, bias=True, K=2):
    ...
    def forward(self, x, kernels=None):
        # softmax_attention: bs, K
        softmax_attention = self.attention(x)
        batch_size, in_planes, height, width = x.size()
        # x: 1, bs*c, wh, ww
        x = x.view(1, -1, height, width)
        # kernel(weight): K, dim*dim*k_size*k_size
        weight = self.weight.view(self.K, -1)

        # aggregate_weight: bs*out_c, in_c, k_size, k_size
        ...
        # TODO check: 此处aggregate_weight的维度操作是否有问题
        # aggregate_weight: bs*out_c, k_size**2, in_c
...
class KernelAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        i = 0
        kernels = []
        for convlayer in self.conv1layers:
            # kernel: bs*out_c, k_size**2, in_c
            _, kernel = convlayer(x_windows[i], kernels=None)
            kernels.append(kernel)
            i = i + 1
        # kernels:  列表中有win_num个 bs*out_c, k_size**2, in_c 的张量
        ...
        # kernels_qkv:  3, bs*out_c, num_heads, win_num*k_size**2, in_c/num_heads
        ...
        # bs*out_c, num_heads, win_num*k_size**2, in_c/num_heads
        kernels_q, kernels_k, kernels_v = kernels_qkv[0], kernels_qkv[1], kernels_qkv[2]
        kernels_q = kernels_q * self.scale

        # attn: bs*out_c, num_heads, win_num*k_size**2, win_num*k_size**2
        ...
        # kernels = self.proj_out(kernels).reshape(B*self.dim, self.kernel_size, self.kernel_size, self.win_num, self.dim).permute(3, 0, 4, 1, 2)



        ### 下面计算SELayer输出并重新进行卷积
        # TODO check: 此处x_windows_origin由bs, c, wh, ww变为1, bs*c, h, w的操作是否正确
        # kernels:  win_num, bs*out_c, in_c, k_size, k_size
        # x_windows_origin:  win_num, 1, bs*c, wh, ww
        i = 0
        x_windows_origin = x_windows_origin.view(self.win_num, 1, -1, x_windows_origin.shape[-2], x_windows_origin.shape[-1])
        x_windows_out = []

        for selayer in self.selayers:
            # kernel:  bs*out_c, in_c, k_size, k_size
...
class SaR_Block(nn.Module):
    def __init__(self, img_size=64, in_chans=32, head=8, win_size=4, norm_layer=nn.LayerNorm, ka_win_size=16, k_size=3):
        """
        input: B x F x H x W
        ...
        self.WA3 = Window_Attention_Reshuffle(dim=self.in_channels, input_resolution=img_size, num_heads=head,
                                    window_size=win_size)
        self.KA = KernelAttention(dim=self.in_channels, num_heads=head, ka_window_size=ka_win_size, kernel_size=k_size, K=2)
...
class PSRT_Block(nn.Module):
    def __init__(self, num=3, img_size=64, in_chans=32, head=8, win_size=8, ka_win_size=16, k_size=3):
        """
        input: B x H x W x F
        :param img_size: size of image
        :param in_chans: feature of image
        ...
        for i_layer in range(self.num_layers):
            layer = SaR_Block(img_size=img_size, in_chans=in_chans, head=head, win_size=win_size//(2**i_layer), ka_win_size=ka_win_size, k_size=k_size)
...
class Block(nn.Module):
    def __init__(self, out_num, inside_num, img_size, in_chans, embed_dim, head, win_size, ka_win_size, k_size):
        super().__init__()
        self.num_layers = out_num
        self.layers = nn.ModuleList()
        self.conv = nn.Conv2d(in_channels=in_chans, out_channels=embed_dim, kernel_size=3, stride=1, padding=1)
        for i_layer in range(self.num_layers):
            layer = PSRT_Block(num=inside_num, img_size=img_size, in_chans=embed_dim, head=head, win_size=win_size, ka_win_size=ka_win_size, k_size=k_size)
            self.layers.append(layer)
...
if __name__ == '__main__':
    ...
    encoder = Block(out_num=2, inside_num=3, img_size=64, in_chans=32, embed_dim=32, head=8, win_size=8, ka_win_size=32, k_size=5)
