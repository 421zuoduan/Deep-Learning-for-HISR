...
class WindowInterAttention(nn.Module):
    ...
    def __init__(self, input_resolution, dim, ka_win_num, k_size, k_stride, k_padding, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.fusion = nn.Conv2d(self.kernels_num*self.dim, self.dim, kernel_size=1, stride=1, padding=0, groups=dim)
    ...
    def forward(self, x):
        ...
        ### 三个全局卷积核和原图做卷积
        # x:  B, (nW+3)*C, H, W
        x = x.permute(0, 3, 1, 2).repeat(1, (self.kernels_num), 1, 1)
        ...
        # global_kernels:  B, (nW+3)*C, k_size, k_size
        kernels = torch.cat([kernels, global_kernel_pool, global_kernel_fusion, global_kernel_sum], dim=1)

        # global_kernels:  B*(nW+3)*C, 1, k_size, k_size
        kernels = kernels.reshape(B*self.kernels_num*C, 1, self.k_size, self.k_size)
