...
class KernelAttention(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, ka_win_num=4, kernel_size=3, stride=1, padding=1, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.convlayer1 = ConvLayer(dim, ka_win_num, kernel_size, stride, padding, groups=self.win_num, k_in=False)
        ...
        self.convlayer2 = ConvLayer(dim, ka_win_num, kernel_size, stride, padding, groups=self.win_num+1, k_in=True)
    ...
    def forward(self, x):
        ...
        kernels = kernels.reshape(self.dim, self.win_num, self.dim, self.kernel_size**2).transpose(2, 3).reshape(self.dim, -1, self.dim)
        ...
        # kernels:  win_num*c, c, k_size, k_size
        kernels = kernels.reshape(self.dim, self.win_num, self.kernel_size, self.kernel_size, self.dim).permute(1, 0, 4, 2, 3).reshape(self.win_num*self.dim, self.dim, self.kernel_size, self.kernel_size)

        # kernels:  (win_num+1)*c, c, k_size, k_size
        kernels = torch.cat([kernels, global_kernel], dim=0)
