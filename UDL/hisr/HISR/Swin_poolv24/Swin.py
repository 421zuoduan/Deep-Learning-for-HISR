...
def pad_feature_map(input, window_size=8):
    ...
    output = padded_feature_map.permute(0, 2, 3, 1)
...
class ConvLayer(nn.Module):
    def __init__(self, dim, k_size, stride, padding, ka_win_num):
        super().__init__()
...
class WindowInterAttention(nn.Module):
    def __init__(self, input_resolution, dim, ka_win_num, k_size, k_stride, k_padding, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.pooling_globalkernel = nn.AdaptiveAvgPool2d((k_size, k_size))
        ...
        self.convlayer = ConvLayer(dim, k_size, k_stride, k_padding, self.nW)
        self.fusion = nn.Conv2d((self.nW+2)*self.dim, self.dim, kernel_size=1, stride=1, padding=0, groups=self.dim)
    ...
    def forward(self, x):
        ...
        ### 生成池化的全局卷积核
        # global_kernel_pool:  B, 1, C, 1, k_size, k_size
        global_kernel_pool = self.pooling_globalkernel(x.permute(0, 3, 1, 2))
        ...
        # x:  B, (nW+2)*C, H, W
        x = x.reshape(B, (self.nW+2), C, H, W).transpose(1, 2).reshape(B, C*(self.nW+2), H, W)
