...
class WindowInterAttention(nn.Module):
    ...
    def __init__(self, input_resolution, dim, ka_win_num, k_size, k_stride, k_padding, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.convlayer = ConvLayer(dim, k_size, k_stride, k_padding, kernels_num=3)
        self.wink_reweight = WinKernel_Reweight(dim, nW=self.nW)
        self.fusion = nn.Conv2d(3*self.dim, self.dim, kernel_size=1, stride=1, padding=0, groups=dim)
    ...
    def forward(self, x):
        ...
        global_kernel_pool = self.pooling(x.permute(0, 3, 1, 2)).reshape(B, 1, C, 1, self.k_size, self.k_size)
        ...
        ### 三个全局卷积核和原图做卷积
        # x:  B, 3*C, H, W
        x = x.permute(0, 3, 1, 2).repeat(1, 3, 1, 1)

        # x:  1, B*3*C, H, W
        x = x.reshape(1, B*3*C, H, W)

        # global_kernels:  B, 3*C, k_size, k_size
        global_kernels = torch.cat([global_kernel_pool, global_kernel_fusion, global_kernel_sum], dim=1)

        # global_kernels:  B*3*C, 1, k_size, k_size
        global_kernels = global_kernels.reshape(B*3*C, 1, self.k_size, self.k_size)

        # x:  1, B*3*C, H, W
        x = self.convlayer(x, global_kernels, B)
        
        # x:  B, 3*C, H, W
        x = x.reshape(B, 3, C, H, W).transpose(1, 2).reshape(B, C*3, H, W)
