...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        if self.shift_size == 0:
            ...
            self.fusion = nn.Conv2d(2*self.dim, self.dim, kernel_size=1, stride=1, padding=0, groups=self.dim)
    ...
    def forward(self, x):
        ...
        if self.shift_size == 0:
            x_wininterattn = self.window_inter_attn(x)  # B H*W C
        ...
        if self.shift_size == 0:
            x = torch.cat([x, x_wininterattn], dim=-1)  # B, H*W, 2C
            x = x.reshape(B, H, W, C, 2).permute(0, 4, 3, 1, 2).reshape(B, 2*C, H, W)
            x = self.fusion(x)
            x = x.permute(0, 2, 3, 1).reshape(B, H*W, C)
