...
class Window_Attention_Shuffle(nn.Module):
    ...
    def forward(self, H, W, x):
        ...
        # shuffle
        x_wa = rearrange(x_wa, 'B H W C -> B C H W')
        x_wa = Win_Shuffle(x_wa, self.window_size)
        x_wa = rearrange(x_wa, 'B C H W -> B H W C')
...
class Window_Attention_Reshuffle(nn.Module):
    ...
    def forward(self, H, W, x):
        ...
        # shuffle
        x_wa = rearrange(x_wa, 'B H W C -> B C H W')
        x_wa = Win_Reshuffle(x_wa, self.window_size)
        x_wa = rearrange(x_wa, 'B C H W -> B H W C')
