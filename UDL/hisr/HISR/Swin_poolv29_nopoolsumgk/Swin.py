...
class WindowInterAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # global_kernels:  B*1*C, 1, k_size, k_size
        global_kernel_fusion = global_kernel_fusion.reshape(B*self.kernels_num*C, 1, self.k_size, self.k_size)

        # x:  1, B*1*C, H, W
        x = self.convlayer(x, global_kernel_fusion, B)
