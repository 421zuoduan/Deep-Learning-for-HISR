...
class TwoHPBlock(nn.Module):
    ...
    def forward(self, x_s, x_h, H, W):
        ...
        p_x = self.norm1(x_s)
        x = x_s + self.attn(p_x, self.norm_h1(x_h), H, W) + self.self_attn(p_x, H, W) # self.drop_path(self.attn(self.norm1(x), H, W))
        x = x + self.ffn(self.norm2(x), H, W)  # self.drop_path(self.mlp(self.norm2(x)))
...
class TwoThreeHBlock(nn.Module):
    ...
    def forward(self, x_s, x_h, H, W):
        ...
        x = p_x + self.self_attn(self.norm_embed(p_x), H, W)
...
class PSABlock(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        if N!=1:
            ...
            # TODO: 输入norm
            x = x + self.linear_encoding(self.norm_pre(x)) # 不受影响，还是共享的，TODO：adaptive
...
class HybridHPBlock(nn.Module):
    ...
    def forward(self, x_s, H, W):
        ...
        # x = x + self.ffn(self.norm2(x), H, W)
        p_x = self.norm1(x_s)
        x = x_s + self.attn(p_x, H, W) + self.self_attn(p_x, H, W) # self.drop_path(self.attn(self.norm1(x), H, W))
        x = x + self.ffn(self.norm2(x), H, W)  # self.drop_path(self.mlp(self.norm2(x)))
