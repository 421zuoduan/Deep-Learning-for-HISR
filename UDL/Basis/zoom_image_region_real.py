...
def statics_PSNR(root_dir, algo_list):
    ...
    os.makedirs("./results_psnr_real", exist_ok=True)
    ...
    for k, files in compared_files.items():
        ...
        for f, gt, my in zip(files, gt_files, my_files):
            ...
            # print(filename, psnr)
            if not os.path.isfile(f"./results_psnr_real/{filename}_{k}.png"):
                # print(f, gt, my)
                try:
                    ...
                    cv2.imwrite(f"./results_psnr_real/{filename}_{k}_{psnr:.2f}.png", cv2.cvtColor(img, cv2.COLOR_RGB2BGR))
...
if __name__ == '__main__':
    ...
    dataset_name = "DDN"
    root_dir = "../../../AdaConv/AdaTrans/my_model_results/real_small" # 4 GT_p21 GT_rain_acc1 GT_rain_014 015
    for i, file in enumerate(os.listdir(root_dir+"/input")):
        print(i, file)
    image_number = [13] # 52 128
    image_number_copy = image_number
    algo_list = ["input", "DSC", "GMM", "Clear", "DDN", "PReNet", "RESCAN", "FBL", "RCDNet", "FuGCN", "IPT", "DFTLW", "DFTLX"]#"input", "Clear", "DDN", "PReNet", "RESCAN", "RCDNet", "FuGCN", "FBL",
    #  17 4 GT_p21 GT_rain_acc1 GT_rain_014 015
    image_number = [13]#[13, 18, 27, 22, 23]
    # algo_list = ["input", "DFTLW", "DFTLX", "FBL", "IPT"]
    ...
    for algo in algo_list:
        ...
        if len(algo_image_files) == 0:
            raise FileNotFoundError(algo)
        if len(algo_image_files) == 2:
            image_number = [1]
        else:
            image_number = image_number_copy
        for num in image_number:
            print(algo_image_files[num])
            compared_images[algo] = cv2.cvtColor(cv2.imread(algo_image_files[num]), cv2.COLOR_BGR2RGB)
    ...
    gen_showImages(compared_images, [
                                      [1420, 900, 120, 30], [180, 250, 120, 30] ], is_save=False) #[0, 20, 50, 50], [120, 60, 50, 50],
    #[120, 120, 80, 30], [240, 60, 80, 30]  [1420, 900, 120, 30], [180, 250, 120, 30] [120, 120, 80, 30], [240, 60, 80, 30]
    # show_compared_image(compared_images,
    #                     [120, 120, 60, 50], # x, y, w, h
