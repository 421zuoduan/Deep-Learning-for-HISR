...
def statics_PSNR(root_dir, algo_list):
    ...
    for algo in algo_list:
        if algo == 'gt':
            algo_dir = os.path.join(root_dir, 'input', "*")
            image_files = glob.glob(algo_dir)
            compared_files['gt'] = []
            # gt_dir = os.path.join(root_dir, 'input', "*")
            # algo_image_files = glob.glob(gt_dir)
            for file in image_files:
                file = file.split('\\')[-1]
                file = file.split('_')[0]
                compared_files['gt'].append(root_dir + '/gt/' + file + '.jpg')
            print(len(compared_files['gt']))
        else:
    ...
    os.makedirs("./results_psnr_DDN", exist_ok=True)
    ...
    for k, files in compared_files.items():
        ...
        for f, gt, my in zip(files, gt_files, my_files):
            ...
            # print(filename, psnr)
            if not os.path.isfile(f"./results_psnr_DDN/{filename}_{k}.png"):
                # print(f, gt, my)
                try:
                    ...
                    cv2.imwrite(f"./results_psnr_DDN/{filename}_{k}_{psnr:.2f}.png", cv2.cvtColor(img, cv2.COLOR_RGB2BGR))
...
if __name__ == '__main__':
    ...
    dataset_name = "DDN"
    root_dir = "../../../AdaConv/AdaTrans/my_model_results/DDN"
    ...
    for algo in algo_list:
        ...
        for num in image_number:
            ...
            if algo == 'gt':
                algo_dir = os.path.join(root_dir, 'input', "*")
                image_files = glob.glob(algo_dir)
                image_files[num - 1] = image_files[num - 1].split('\\')[-1]
                image_files[num - 1] = image_files[num - 1].split('_')[0]
                gt_file = root_dir + '/gt/' + image_files[num - 1] + '.jpg'
                print(gt_file)
                compared_images['gt'] = cv2.cvtColor(cv2.imread(gt_file), cv2.COLOR_BGR2RGB)
            else:
