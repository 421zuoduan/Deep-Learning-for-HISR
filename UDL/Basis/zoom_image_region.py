...
def statics_PSNR(root_dir, algo_list):
    ...
    os.makedirs("./results_psnr", exist_ok=True)

    gt_files = compared_files.pop('gt')
    my_files = compared_files['RestormerUp']
    psnr_dicts = {}
    for k, files in compared_files.items():
        psnr_dicts[k] = []
        for f, gt, my in zip(files, gt_files, my_files):
            gt, img, my = cv2.cvtColor(cv2.imread(gt), cv2.COLOR_BGR2RGB), cv2.cvtColor(cv2.imread(f), cv2.COLOR_BGR2RGB), \
                          cv2.cvtColor(cv2.imread(my), cv2.COLOR_BGR2RGB)
            aem = np.abs(my - img) ** 2
            psnr = PSNR(gt, img)
            filename = f.split('\\')[-1][:-4]
            print(filename, psnr)
            cv2.imwrite(f"./results_psnr/{filename}_{k}_{psnr:.2f}.png", cv2.cvtColor(img, cv2.COLOR_RGB2BGR))
            cv2.imwrite(f"./results_psnr/{filename}_{k}.png", cv2.cvtColor(aem, cv2.COLOR_RGB2GRAY))
...
if __name__ == '__main__':
    ...
    algo_list = ["gt", "input",  "DSC", "JCAS", "Clear", "DDN", "PReNet", "RESCAN", "RCDNet", "FBL", "FuGCN", "IPT", "DFTLW1575", "RestormerUp"]
    ...
    image_number = [129] # 52 128 14
    image_number_copy = image_number
    # algo_list = ["gt", "input", "DSC", "jorderE", "Clear", "DDN", "PReNet", "RESCAN", "RCDNet", "FBL", "FuGCN", "IPT", "RestormerUp", "RestormerUp"]#"input", "Clear", "DDN", "PReNet", "RESCAN", "RCDNet", "FuGCN", "FBL",
    # algo_list = ["gt", "input", "FuGCN", "IPT", "Restormer", "DFTL-v", "RestormerUp"]
    # dataset_name = "Rain200H"
    # root_dir = "../../../AdaConv/AdaTrans/my_model_results/Rain200H"
    # image_number = [129] # 51 129
    # algo_list = ["gt", "FBL", "Restormer", "DFTL-0029", "RestormerUp"]
    ...
    for algo in algo_list:
        ...
        for num in image_number:
            ...
            if algo in compared_images.keys():
                algo = 'DFTL-W'
