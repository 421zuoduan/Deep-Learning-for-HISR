...
def get_scheduler(optimizer, cur_ep=-1):
    if 'lambda' == 'lambda':
        def lambda_rule(ep):
            lr_l = 1.0 - max(0, ep - (-1)) / float(-1 - (-1) + 1)
            return lr_l
        scheduler = lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda_rule, last_epoch=-1)
    elif 'lambda' == 'step':
        scheduler = lr_scheduler.StepLR(optimizer, step_size=-1, gamma=0.1, last_epoch=cur_ep)
    #  SGDR: Stochastic Gradient Descent with Warm Restarts- CosineAnnealingWarmRestarts
    elif 'lambda' == 'CosineAnnealingWarmRestarts':
...
if __name__ == '__main__':
    ...
    summary(NetDis(opt=1).cuda(),input_size=[(1, 128, 160, 128)],device='cuda')