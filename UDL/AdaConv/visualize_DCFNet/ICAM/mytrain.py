...
def main():
    ...
    device = 1 if torch.cuda.is_available() and True else 'cpu'
    name = 'biobank_age' + '_' + time.strftime("%d%m%Y-%H%M")
    results_path = os.path.join('./results', name)
    image_size = IMAGE_SIZE
    age_range_0 = AGE_RANGE_0
    ...
    total_it = 0
    n_ep=300
    val_accuracy = np.zeros(n_ep)
    val_f1 = np.zeros(n_ep)
    val_precision = np.zeros(n_ep)
    val_recall = np.zeros(n_ep)
    val_cross_corr_a = np.zeros(n_ep)
    val_cross_corr_b = np.zeros(n_ep)
    val_mse = np.zeros(n_ep)
    val_mae = np.zeros(n_ep)
    t0 = time.time()
    data_dim='3d'
    ...
    anomaly_dataloader, anomaly_val_dataloader, anomaly_test_dataloader = _load_dataloader()

    print('\n--- load model ---')
    model = ICAM()
    model.setgpu(device)
    model.initialize()
    model.set_scheduler(last_ep=ep0)
    # save_opts = vars(opts)
    saver = Saver(results_path,name,1000)

    if not os.path.exists(results_path):
        os.makedirs(results_path)

    # with open(results_path + '/parameters.json', 'w') as file:
    ...
    for ep in range(ep0, n_ep):
        ...
        while iter_counter < len(anomaly_dataloader) and iter_counter < len(healthy_dataloader):
            ...
            healthy_images, healthy_label_reg, healthy_mask = torch.randn([1,1,32, 32, 32],dtype=torch.float32), torch.randn([1,1,32, 32, 32],dtype=torch.float32), torch.randn([2,1,32, 32, 32],dtype=torch.float32)
            healthy_c_org = torch.zeros((healthy_images.size(0), num_domains)).to(device)
            healthy_c_org[:, 0] = 1
            anomaly_c_org = torch.zeros((healthy_images.size(0), num_domains)).to(device)
            anomaly_c_org[:, 1] = 1
            ...
            c_org = c_org.to(device).detach()
            label_reg = label_reg.to(device).detach()

            # update model
            if (iter_counter % 3) != 0 and iter_counter < len(anomaly_dataloader) - opts.d_iter:
                model.update_D_content( images, c_org)
                continue

            model.update_D(images, c_org, label_reg, mask=mask)
            model.update_EG()
...
def _load_dataloader():
    ...
    """
    data_type='biobank_age'
    batch_size=2
    val_batch_size=2
    if data_type == 'syn2d':
        healthy_dataloader = init_synth_dataloader(
            1, anomaly=False, mode='train', batch_size=batch_size // 2)
        anomaly_dataloader = init_synth_dataloader(
            1, anomaly=True, mode='train', batch_size=batch_size // 2)
        healthy_val_dataloader = init_synth_dataloader(
            1, anomaly=False, mode='val', batch_size=val_batch_size // 2)
        anomaly_val_dataloader = init_synth_dataloader(
            1, anomaly=True, mode='val', batch_size=val_batch_size // 2)
        healthy_test_dataloader = init_synth_dataloader(
            1, anomaly=False, mode='test', batch_size=val_batch_size // 2)
        anomaly_test_dataloader = init_synth_dataloader(
            1, anomaly=True, mode='test', batch_size=val_batch_size // 2)
    elif data_type == 'biobank_age':
        healthy_dataloader, healthy_val_dataloader, healthy_test_dataloader, \
