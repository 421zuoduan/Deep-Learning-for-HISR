...
class ICAM(nn.Module):
    def __init__(self, opts=1):
        ...
        self.dis1 = networks.NetDis(1)
        self.dis2 = networks.NetDis(1)
        self.enc_c = networks.NetEc(1)
        self.gen = networks.NetGen(1)
        self.enc_a = networks.NetEa(1)
        self.disContent = networks.NetDisContent(1)

        self.dis1_opt = torch.optim.Adam(self.dis1.parameters(), lr=lr, betas=(betas[0], betas[1]), weight_decay=opt_weight_decay)
        self.dis2_opt = torch.optim.Adam(self.dis2.parameters(), lr=lr, betas=(betas[0], betas[1]), weight_decay=opt_weight_decay)
        self.enc_c_opt = torch.optim.Adam(self.enc_c.parameters(), lr=lr, betas=(betas[0], betas[1]), weight_decay=opt_weight_decay)
        self.enc_a_opt = torch.optim.Adam(self.enc_a.parameters(), lr=lr, betas=(betas[0], betas[1]), weight_decay=opt_weight_decay)
        self.gen_opt = torch.optim.Adam(self.gen.parameters(), lr=lr, betas=(betas[0], betas[1]), weight_decay=opt_weight_decay)
        self.disContent_opt = torch.optim.Adam(self.disContent.parameters(), lr=lr_dcontent, betas=(betas[0], betas[1]), weight_decay=opt_weight_decay)

        # loss functions
        self.cls_loss = nn.BCEWithLogitsLoss()
        if self.lambda_l2_rec > 0:
    ...
    def set_scheduler(self, last_ep=0):
        ...
        :return:
        """
        self.dis1_sch = networks.get_scheduler(self.dis1_opt, last_ep)
        self.dis2_sch = networks.get_scheduler(self.dis2_opt, last_ep)
        self.disContent_sch = networks.get_scheduler(self.disContent_opt, last_ep)
        self.enc_c_sch = networks.get_scheduler(self.enc_c_opt, last_ep)
        self.enc_a_sch = networks.get_scheduler(self.enc_a_opt, last_ep)
        self.gen_sch = networks.get_scheduler(self.gen_opt, last_ep)
    ...
    def backward_D(self, netD, real, fake):
        ...
        loss_D_gan = 1 * loss_D_gan
        loss_D_cls = 1 * loss_D_cls
    ...
    def backward_EG(self):
        ...
        # self recon
        loss_G = torch.mean(
            torch.abs(self.input - torch.cat((self.fake_AA_encoded, self.fake_BB_encoded), 0))) * 100
        self.l1_self_rec_loss = loss_G.item()

        # l2 recon + cyclic
        if 100 > 0:
            l2_self_rec_loss = self.l2_loss(self.input, torch.cat((self.fake_AA_encoded, self.fake_BB_encoded), 0)) * 100
            self.l2_self_rec_loss = l2_self_rec_loss.item()
            loss_G += l2_self_rec_loss
        if 100 > 0:
            l2_cc_rec_loss = self.l2_loss(self.input, torch.cat((self.fake_A_recon, self.fake_B_recon), 0)) * 100
        ...
        loss_G_gan = loss_G_GAN * 1
        self.G_gan_loss = loss_G_gan.item()
        loss_G += loss_G_gan

        # classification
        loss_G_cls = self.cls_loss(pred_fake_cls, self.c_org) * 5
        self.G_gan_cls_loss = loss_G_cls.item()
        loss_G += loss_G_cls

        #cross-cycle recon
        loss_G_L1_cc = torch.mean(
            torch.abs(self.input - torch.cat((self.fake_A_recon, self.fake_B_recon), 0))) * 100
        loss_G += loss_G_L1_cc
        ...
        loss_E_cls_self = self.cls_loss(self.E_pred_cls, self.c_org) * 10
        self.E_cls_self_loss = loss_E_cls_self.item()

        # regression loss on E_a
        if False:
            loss_E_reg_self = self.reg_loss(self.E_pred_reg, self.c_reg) * 10
        ...
        if True:
            diff_M_reg_loss = torch.abs(torch.cat((self.diff_fake_A_encoded,
                                                   self.diff_fake_B_encoded),0)).mean() * 10
    ...
    def backward_E_content(self, z_content):
        ...
        if not (0.5 == 0):
            ...
            all1 = 0.5 * torch.ones_like(self.c_org).to(self.device)
            loss_E_content = self.cls_loss(pred_cls, all1) * 1
        else:
            # the goal is to fool discriminator- i.e. reverse the classes
            loss_E_content = self.cls_loss(pred_cls, 1 - self.c_org) * 1
    ...
    def backward_G_alone(self):
        ...
        # classification
        loss_G_cls2 = self.cls_loss(pred_fake_cls, self.c_org) * 5
        self.G_gan2_cls_loss = loss_G_cls2.item()

        loss_G_GAN2 = 1 * loss_G_GAN2
        ...
        if True:
            loss_z_L1_a = torch.mean(torch.abs(self.mu2_a - self.z_random_a)) * 1
            loss_z_L1_b = torch.mean(torch.abs(self.mu2_b - self.z_random_b)) * 1
...
if __name__ == '__main__':
    ...
    summary(ICAM(),input_size=[(1,16,20,16)],batch_size=1,device='cuda')
