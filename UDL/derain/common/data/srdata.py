import os
import glob
from UDL.derain.common.data import common
import pickle
import numpy as np
import imageio
import torch
...
class SRData(data.Dataset):
    def __init__(self, args, name='', train=True, benchmark=False):
        ...
        if test_every > 0:
            self.repeat \
                = args.test_every / (len(self.images_hr) / args.samples_per_gpu)
            print(f"repeat {test_every} and bs: {args.samples_per_gpu}")
        else:
            print(f"repeat {test_every}")
            self.repeat = test_every
    ...
    def __getitem__(self, idx):
        ...
        return {'O': lr_tensor, 'B': hr_tensor, 'filename': filename}
    ...
    def _load_file(self, idx):
        ...
        if self.args.ext.find('bin') >= 0:
            ...
            if self.args.ext == 'img' or self.benchmark:
                ...
                with open(f_hr, 'rb') as _f: hr = np.load(_f, allow_pickle=True)[0]['image'] / 255.0
                with open(f_lr, 'rb') as _f: lr = np.load(_f, allow_pickle=True)[0]['image'] / 255.0
    ...
    def get_patch(self, lr, hr):
        ...
        if self.train:
           # print('****preparte data****')
            ...
            # lr, hr = common.resize_image(lr, hr, patch_size=args.patch_size)
