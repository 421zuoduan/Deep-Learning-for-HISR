...
import datetime
from DFTL_IPT_hybrid_model import DFTL as net
...
def build(args):
    ...
    model = net(args, image_size=args.patch_size, in_channels=3, out_channels=3,
              ...
              hidden_dim=[32, 64, 128, 256, 480], num_heads=[1, 2, 4, 8], mlp_ratios=[4, 4, 4, 4],
...
class EpochRunner():
    ...
    def eval(self, eval_loader, model, criterion,):
    ...
    def eval_framework(self, eval_loader, model, criterion):
        ...
        for batch, idx in metric_logger.log_every(eval_loader, 1, header):
            ...
            outputs = quantize(add_mean(outputs), 255)
            normalized = outputs[0].mul(255 / args.rgb_range)
            tensor_cpu = normalized.byte().permute(1, 2, 0).cpu()
            imageio.imwrite(os.path.join("./my_model_results", ''.join([filename[0][:-2], '.png'])),
                            tensor_cpu.numpy())
...
def main(args):
    ...
    if args.launcher == 'none' or args.eval:
    ...
    ##################################################
    if args.eval:
        eval_loader = sess.get_eval_dataloader('eval')
        runner.eval(eval_loader, model, criterion)
    else:
        ...
        runner.run(train_loader, model, criterion, optimizer, None, None, train_sampler=train_sampler)
...
if __name__ == "__main__":
    ...
    model_path = './results/Fu100L/HPT/hybridIPTTwoAttn_sAs/model_2021-08-24-18-39/9.pth.tar'
    ...
    parser.add_argument('--crop_batch_size', type=int, default=48,
    ...
    args.experimental_desc = "hybridIPTTwoAttn_sAs"
    args.dataset = "Fu100L"
