...
from san_pvt import HPT as net
...
class BCMSLoss(torch.nn.Module):
    ...
    def forward(self, x, gt):
        ...
        loss = _ssim_loss.reshape(-1, 1, 1, 1) * (
               ...
               l2_loss * ((_ssim_loss.reshape(-1, 1, 1, 1) / w_s) ** ind + (l1_loss / w_1) ** ind + 1e-8) ** ind# + (
...
def build(args):
    ...
    # model = net(
    #         patch_size=128, in_channels=3, out_channels=3, patch_sride=[2, 2, 2, 2],
    #         hidden_dim=[32, 64, 128, 320, 512], num_heads=[1, 2, 4, 8], mlp_ratios=[8, 8, 4, 4],
    #         depths=[2, 2, 2, 2], sr_ratio=
...
def train_one_epoch(model, criterion, data_loader, optimizer, device, epoch, scaler):
    ...
    for batch, idx in metric_logger.log_every(data_loader, print_freq, header):
        ...
        outputs, loss_dicts = model(samples, gt, args.batch_size, index)
...
def load_checkpoint(args, model, optimizer):
    if args.resume:
        ckpt = partial_load_checkpoint(torch.load(args.resume)['state_dict'])
        if ckpt is not None:
...
class EpochRunner():
    ...
    def run(self, train_sampler, train_loader, model, criterion, optimizer, val_loader, scheduler):
        ...
        model = model_amp(self.args, model, criterion)
        optimizer, scaler = model.apex_initialize(optimizer)
        model, optimizer = load_checkpoint(self.args, model, optimizer)
    ...
    def validate_framework(self, val_loader, model, criterion, epoch=0):
        ...
        for batch, idx in metric_logger.log_every(val_loader, 1, header):
            ...
            outputs, loss_dicts = model(samples, gt, 1, index)
...
if __name__ == "__main__":
    ...
    model_path = './results/100L/HPT/44_res/model_2021-07-12-16-34/91.pth.tar'
    ...
    parser.add_argument('--lr', default=2e-4, type=float)#1e-4
    # parser.add_argument('--lr_backbone', default=1e-5, type=float)
    # parser.add_argument('--weight_decay', default=1e-4, type=float)
    parser.add_argument('-b', '--batch-size', default=3, type=int,  # 8
    ...
    args.experimental_desc = "44_res"#layernorm_100L
    args.dataset = "100L"
