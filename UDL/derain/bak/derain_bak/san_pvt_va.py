...
class DWConv(nn.Module):
    ...
    def forward(self, x):
        b, p, N, C = x.shape
        L = int(math.sqrt(N))
        x = x.reshape(-1, L, L, C).permute(0, 3, 1, 2)
        x = self.dwconv(x)
...
class APUNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.up4(x, x0)
        x = self.norm(x)
        # meanval = x.mean(dim=[2, 3], keepdim=True)
...
class Pvt(nn.Module):
    def __init__(self, batch_size, patch_size, patch_sride, in_channels, out_channels, hidden_dim,
        ...
        self.patch_embed1 = PatchConvEmbed(batch_size=batch_size, img_size=patch_size, patch_sride=patch_sride[0], in_chans=hidden_dim[0],
                                           embed_dim=hidden_dim[1])
        self.patch_embed2 = PatchConvEmbed(batch_size=batch_size, img_size=patch_size // 2, patch_sride=patch_sride[1], in_chans=hidden_dim[1],
                                           embed_dim=hidden_dim[2])
        self.patch_embed3 = PatchConvEmbed(batch_size=batch_size, img_size=patch_size // 4, patch_sride=patch_sride[2], in_chans=hidden_dim[2],
                                           embed_dim=hidden_dim[3])
        self.patch_embed4 = PatchConvEmbed(batch_size=batch_size, img_size=patch_size // 8, patch_sride=patch_sride[3], in_chans=hidden_dim[3],
                                           embed_dim=hidden_dim[4])
        # self.patch_embed5 = PatchConvEmbed(img_size=3, patch_size=2, in_chans=256,
        #                                     embed_dim=256)
    ...
    def forward_post(self, x):
        ...
        x0 = self.inc(x)#32
        # stage 1 -64
        x, (H, W, C) = self.patch_embed1(x0)  # x
        x = x + self.pos_embed1
        for blk in self.down1:
            x = blk(x, H, W)
        x1 = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
...
class BIMBlockND(nn.Module):
    ...
    def forward_NPk_j(self, x):
        ...
        if g_h != x_h:
            ...
            x = F.interpolate(x, size=_pair(int(g_h)), mode='bilinear', align_corners=True)
...
def test_net():
    ...
    # net = Pvt(patch_size=48, in_channels=3, out_channels=3, patch_sride=[2, 2, 2, 2],
    #         hidden_dim=[32, 64, 128, 320, 512], num_heads=[1, 2, 4, 8], mlp_ratios=[8, 8, 4, 4],
    #         depths=[2, 2, 2, 2], sr_ratio=[1, 1, 1, 1])
    net = HPT(1, patch_size=[torch.tensor([2, 2]), torch.tensor([2, 2])], image_size=48, in_channels=3, out_channels=3, patch_stride=[2, 2, 2, 2],
              hidden_dim=[32, 64, 128, 320, 512], num_heads=[1, 2, 4, 8], mlp_ratios=[8, 8, 4, 4],
              depths=[2, 2, 2, 2], sr_ratio=[1, 1, 1, 1])
...
class HBlock(nn.Module):
    ...
    def __init__(self, downscaling_factor, norm_dim, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, sr_ratio=1):
        super().__init__()
        self.norm1 = norm_layer(norm_dim)#dim
        # the effect of sr_ratio
    ...
    def forward(self, x, bs, axes=[2, 3]): # h, w
        ...
        if 1 in self.downscaling_factor:
            for axis, half, factor in zip(axes, shape, self.downscaling_factor):
                ...
                if axis == 2:
                    x = x.unfold(axis, half, half).transpose(-1, -2)
                elif axis == 3:
                    x = x.unfold(axis, half, half).transpose(3, 2)
            ...
            x = x.unfold(axes[0], new_h, new_h).unfold(axes[1], new_w, new_w).\
...
class HPT(nn.Module):
    def __init__(self, image_size, patch_size, patch_stride, in_channels, out_channels, hidden_dim,
        ...
        num_patches = torch.prod(patch_num[0]) ** 1
        self.down1 = nn.ModuleList([HBlock(downscaling_factor= num_patches, norm_dim=[num_patches, N // num_patches, hidden_dim[1]],
            dim=hidden_dim[1], num_heads=num_heads[0], mlp_ratio=mlp_ratios[0], qkv_bias=True, qk_scale=None, norm_layer=partial(nn.LayerNorm, eps=1e-6),
            sr_ratio=sr_ratio[0])
            for i in range(depths[0])])
        num_patches_base = torch.prod(patch_num[1])
        num_patches1 = num_patches_base ** 2
        self.down2 = nn.ModuleList([HBlock(downscaling_factor=num_patches, norm_dim=[num_patches1, N // num_patches1, hidden_dim[2]],
            dim=hidden_dim[2], num_heads=num_heads[1], mlp_ratio=mlp_ratios[1], qkv_bias=True, qk_scale=None, norm_layer=partial(nn.LayerNorm, eps=1e-6),
            sr_ratio=sr_ratio[1])
            for i in range(depths[1])])
        num_patches = num_patches_base * num_patches1
        self.down3 = nn.ModuleList([HBlock(downscaling_factor=1, norm_dim=[num_patches1, N // num_patches, hidden_dim[3]],#[k ** 3 for k in patch_num],
            dim=hidden_dim[3], num_heads=num_heads[2], mlp_ratio=mlp_ratios[2], qkv_bias=True, qk_scale=None, norm_layer=partial(nn.LayerNorm, eps=1e-6),
            sr_ratio=sr_ratio[2])
            for i in range(depths[2])])
        factor = 2 if bilinear else 1
        ...
        self.down4 = nn.ModuleList([HBlock(downscaling_factor=1, norm_dim=[num_patches1, N // num_patches, hidden_dim[4]],#[k ** 4 for k in patch_num],
            dim=hidden_dim[4], num_heads=num_heads[3], mlp_ratio=mlp_ratios[3], qkv_bias=True, qk_scale=None, norm_layer=partial(nn.LayerNorm, eps=1e-6),
            sr_ratio=sr_ratio[3])
            for i in range(depths[3])])
    ...
    def forward_pre(self, x, bs):
        ...

    def forward_post(self, I, bs):
        # x = x.view(x.shape[0] * x.shape[1], x.shape[2], x.shape[3], x.shape[4])
    ...
    def forward(self, x, bs):
        ...
        if self.norm_pre:
            return self.forward_pre(x, bs)
        return self.forward_post(x, bs)
