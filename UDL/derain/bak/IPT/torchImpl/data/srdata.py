...
import numpy as np
import imageio
from IPT.torchImpl.data import common
...
class SRData:
    ...
    def _repeat(self, args):
        """srdata"""
        n_patches = args.batch_size * args.test_every#8000 // 2000
        n_images = len(self.images_hr) #len(args.data_train) is list
        if n_images == 0:
            self.repeat = 0
        else:
            self.repeat = max(n_patches // n_images, 1)
        print("trainData_repeat:", n_patches, n_images)
