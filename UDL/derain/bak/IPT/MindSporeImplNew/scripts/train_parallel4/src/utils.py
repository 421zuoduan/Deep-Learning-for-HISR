...
class Trainer():
    ...
    def train(self, epoch, steps):
        ...
        for batch_idx, imgs in enumerate(self.trainloader):
            ...
            print('Epoch: %g, Task: %g, [%g]/[%g], loss: %f, scaling factor:%f, time: %f s, overflow: %s' % \
                (epoch, idx.shape[0], batch_idx, steps, loss.asnumpy(), sens.asnumpy(), t2 - t1, overflow), flush=True)
