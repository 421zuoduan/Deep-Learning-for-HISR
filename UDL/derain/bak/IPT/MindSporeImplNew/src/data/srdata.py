...
class SRData:
    ...
    def __init__(self, args, name='', train=True, benchmark=False):
        ...
        if self.args.derain:
            if self.train:
                ...
                # print("norain len:", len(self.clear_train))
                self.rain_train = []
                ...
                # self.derain_test = os.path.join(args.dir_data, "Rain100L")
                # # self.derain_lr_test = common.search(self.derain_test, "rainy")
                # self.derain_lr_test = common.search(self.derain_test, "rain")
                # # self.derain_hr_test = [path.replace("rainy/", "no") for path in self.derain_lr_test]
                # self.derain_hr_test = [path.replace("rainy", "norain") for path in self.derain_lr_test]
    ...
    def _repeat(self, args):
        """srdata"""
        # 16 * 1000 // len(RainTrainL) * 200 = 8 200*8/16 = 100
        ...
        print("trainData_repeat:", n_patches, n_images, len(args.data_train), len(self.images_hr))
    ...
    def __len__(self):
        if self.train:
            print("len:", len(self.images_hr) * self.repeat)
            return len(self.images_hr) * self.repeat

        if self.args.derain and not self.args.alltask:
            return len(self.derain_hr_test)
    ...
    def get_patch(self, lr, hr):
        ...
        # print("scale:", scale, self.scale, self.idx_scale)
        if self.train:
...
# file_list = os.listdir('/IPT/MindSporeImpl/cache/data/Rain100L/rain/X2')
# print(file_list[0])
# def replace_name(file):
#     img = imageio.imread(os.path.join('/IPT/MindSporeImpl/cache/data/Rain100L/rain/X2', file))
#     new_file = ''.join([file.split('.')[0][:-2], '.png']).replace('norain', 'rainy')
#     print(file, new_file)
#     imageio.imwrite(os.path.join('/IPT/MindSporeImpl/cache/data/Rain100L/tmp', new_file), img)
