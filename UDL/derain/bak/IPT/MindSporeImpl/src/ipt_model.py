...
class MultiheadAttention(nn.Cell):
    ...
    def construct(self, tensor_q, tensor_k, tensor_v):
        ...
        attention_probs = self.softmax_cast(attention_probs, mstype.float32)#mstype.float16
        if self.use_dropout:
...
class IPT(nn.Cell):
    ...
    def __init__(self, args, conv=default_conv):
        ...
        self.dytpe = mstype.float32#mstype.float16
        self.scale_idx = 0
        ...
        self.tail = nn.CellList([
            nn.SequentialCell(Upsampler(conv, s, n_feats).to_float(self.dytpe),
                              conv(n_feats, args.n_colors, kernel_size).to_float(self.dytpe)) \
                                  for s in self.args.scale])#[2, 3, 4, 1, 1, 1]
...
class IPT_post():
    ...
    def forward(self, x, idx, shave=12, batchsize=64):
        ...
        y_hw_cut = self.cast(y_hw_cut, mstype.float32)
        ...
        y_w_cut = self.cut_w_new(x_w_cut, h, w, h_cut,
                                 w_cut, padsize, shave, scale, batchsize)
        # y_h_cut = self.cast(y_h_cut, mstype.float32)
        y_w_cut = self.cast(y_w_cut, mstype.float32)
