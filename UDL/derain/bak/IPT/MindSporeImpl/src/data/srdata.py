...
class SRData:
    ...
    def __init__(self, args, name='', train=True, benchmark=False):
        ...
        if self.args.derain:
            if self.train:
                ...
                print(args.dir_data, os.listdir(args.dir_data))
        ...
        if args.ext == 'img' or self.benchmark:
            ...
            for s in self.scale:
                if s == 1:
                    ...
                    os.makedirs(
                        os.path.join(self.dir_lr, 'X{}'.format(s)), exist_ok=True)

            self.images_hr, self.images_lr = [], [[] for _ in self.scale]
            # print("---------"*20)
            for h in list_hr:
                # b = h.replace(self.apath, path_bin)
                b = h.replace(self.ext[0], '.pt')
                if 'x2' not in b:
    ...
    def _scan(self):
        """srdata"""
        names_hr = sorted(
            glob.glob(os.path.join(self.dir_hr, 'norain/*' + self.ext[0])))
