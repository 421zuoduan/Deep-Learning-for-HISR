...
from common.derain_dataset import derainSession, PSNR_ycbcr, add_mean, sub_mean, quantize
...
def load_checkpoint(args, model, optimizer, ignore_params=[]):
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            #partial_load_checkpoint(checkpoint['state_dict'], args.amp, ignore_params)
            # if args.distributed:
...
class EpochRunner():
    ...
    def eval_framework(self, eval_loader, model, criterion):
        ...
        psnr_list = []
        # for iteration, batch in enumerate(val_loader, 1):
        saved_path = f"./my_model_results/uformer/{args.dataset}"
        ...
        for batch, idx in metric_logger.log_every(eval_loader, 1, header):
            ...
            psnr = psnr_v2(add_mean(outputs), gt * 255, 4, 255.0)
...
if __name__ == "__main__":
    ...
    #vit
    #train
    #- Averaged stats: lr: 0.000013  grad_norm: 0.222596  Loss: 0.0072 (0.0096847)  psnr: 36.5447 (34.2029583)
    #-  * Best validation psnr so far@1 34.2029583 in epoch 319 ->-  Averaged stats: ssim: 0.9660 (0.9542172)  psnr: 36.4662 (34.9000542) (34.900054168701175)
