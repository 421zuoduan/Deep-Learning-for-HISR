...
from auxiliary import MetricLogger, SmoothedValue, set_random_seed, create_logger, log_string
import torch
...
from common.derain_dataset import derainSession, add_mean, sub_mean, quantize, PSNR_ycbcr
import imageio
import datetime
...
def build_backbone(args):
    ...
    if args.position_embedding != "None":
...
class DETRsegm(nn.Module):
    def __init__(self, detr, out_channel=3, freeze_detr=False, decode=True):
        ...
        self.mask_head = MaskHeadSmallConv(hidden_dim, [1024, 512, 256], hidden_dim,
    ...
    def forward(self, samples):
        ...
        if self.decode:
            dec_output, enc_output = self.detr.transformer(src_proj, mask, self.detr.query_embed.weight, None)#pos[-1]
        else:
...
def build(args):
    ...
    # model = build_newtransformer(args)
...
def train_one_epoch(model, criterion, data_loader, optimizer, device, epoch, max_norm):
    # model = model_amp(args, model, criterion, optimizer)
...
class EpochRunner():
    ...
    def eval_framework(self, eval_loader, model, criterion):
        ...
        for batch, idx in metric_logger.log_every(eval_loader, 1, header):
            ...
            normalized = pred[0].mul(255 / 255.0)
            ...
            psnr = psnr_y(add_mean(outputs), gt * 255, 4, 255.0)
