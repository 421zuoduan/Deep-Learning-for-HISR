...
class SwinIR(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            x = self.conv_after_body(self.forward_features(x)) + x
            x = self.conv_before_upsample(x)
            x = self.lrelu(self.conv_up1(x))#torch.nn.functional.interpolate(x, scale_factor=2, mode='nearest')))
            x = self.lrelu(self.conv_up2(x))#torch.nn.functional.interpolate(x, scale_factor=2, mode='nearest')))
...
if __name__ == '__main__':
    ...
    model = SwinIR(upscale=1, img_size=(height, width),
                   ...
                   embed_dim=60, num_heads=[6, 6, 6, 6], mlp_ratio=2, upsampler='nearest+conv')#pixelshuffledirect
    print(model)
    ...
    print(x.shape)
    '''
    D:\anaconda\envs\mmlab\python.exe D:/ProjectSets/NDA/Attention/SwinIR/models/network_swinir.py
SwinIR(
