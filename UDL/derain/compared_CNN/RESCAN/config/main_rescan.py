...
class SetCriterion(nn.Module):
    ...
    def __init__(self, losses, weight_dict):
        ...
        self.ssim = SSIM(data_range=1).cuda()
        self.loss_dicts = {}
    ...
    def forward(self, outputs, targets, *args, **kwargs):
        ...
        loss_list = [self.crit(O_R, R) for O_R in outputs]
        ssim_list = [self.ssim(O - O_R, O - R) for O_R in outputs]
...
def train_one_epoch(args, model, criterion, data_loader, optimizer, device, epoch, scaler):
    ...
    for batch, idx in metric_logger.log_every(data_loader, args.print_freq, header):
        ...
        outputs, loss_dicts = model(samples, gt, O=samples)
        ...
        pred = outputs[-1]#add_mean(outputs[-1])
        metric_logger.update(psnr=reduce_mean(psnr_v2(pred * 255.0, gt * 255.0, 4, 255.0))) #reduce_mean(psnr_v2(add_mean(outputs), gt, 4, 255.0))
...
def main(args):
    ...
    if args.eval:
        ...
        if args.lr_scheduler:
            ...
            scheduler.lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[150, 175], gamma=0.1)
