...
class SetCriterion(nn.Module):
    ...
    def __init__(self, args, losses, weight_dict):
        ...
        self.losses = losses
        self.losses.start_log()
        self.loss_dicts = {}

        self.loss_Bs_all = SmoothedValue(window_size=1, fmt='{value:.6f}')
        self.loss_Rs_all = SmoothedValue(window_size=1, fmt='{value:.6f}')
        self.loss_B_all = SmoothedValue(window_size=1, fmt='{value:.6f}')
        self.loss_R_all = SmoothedValue(window_size=1, fmt='{value:.6f}')
        self.S = args.stage

    def reset_loss(self):
        self.loss_Bs_all.reset(1)
        self.loss_Rs_all.reset(1)
        self.loss_B_all.reset(1)
        self.loss_R_all.reset(1)
    ...
    def forward(self, outputs, targets, *args, **kwargs):
        ...
        B0, ListB, ListR = outputs
        # print(B0[0, 0, 0, 0], ListB[0][0, 0, 0, 0], ListR[0][0, 0, 0, 0])
        ...
        for j in range(self.S):
            loss_Bs = float(loss_Bs) + 0.1 * loss(ListB[j], hr)
            loss_Rs = float(loss_Rs) + 0.1 * loss(ListR[j], lr - hr)
        loss_B = loss(ListB[-1], hr)
        loss_R = 0.9 * loss(ListR[-1], lr - hr)
        loss_B0 = 0.1 * loss(B0, hr)
        loss_dicts = loss_B0 + loss_Bs + loss_Rs + loss_B + loss_R
        self.loss_Bs_all.update(loss_Bs)
        self.loss_B_all.update(loss_B)
        self.loss_Rs_all.update(loss_Rs)
        self.loss_R_all.update(loss_R)

        self.loss_dicts.update({'Loss': loss_dicts,
                                'loss_Bs_all': self.loss_Bs_all.total,
                                'loss_B_all': self.loss_B_all.total,
                                'loss_Rs_all': self.loss_Rs_all.total,
                                'loss_R_all': self.loss_R_all.total})
        # print(f"log: {loss.log[-1][0]}, Loss: {loss_dicts.item()}, loss_Bs_all: {self.loss_Bs_all.total}, "
        #       f"loss_B_all: {self.loss_B_all.total}, loss_Rs_all: {self.loss_Rs_all.total}"
        #       f"loss_R_all: {self.loss_R_all.total}")
...
# arch
##########################################################################################
import model as pre_model
import loss as pre_loss


def build(args):
    device = torch.device(args.device)

    model = pre_model.Model(args, None).cuda()
    # 造成日志无法使用
    ...
    loss = pre_loss.Loss(args, None)  # if not args.test_only else None
    weight_dict = {'Loss': 1}
    # losses = {'Loss': loss}
...
def train_one_epoch(args, model, criterion, data_loader, optimizer, device, epoch, scaler):
    ...
    for batch, idx in metric_logger.log_every(data_loader, args.print_freq, header):
        ...
        # plt.savefig(f"{idx}_{filename[0]}_m.png")

        outputs, loss_dicts = model(samples * 255.0, gt * 255.0, O=samples * 255.0)
        losses = loss_dicts['Loss']

        losses = losses / args.accumulated_step

        if losses.item() < args.skip_threshold * getattr(metric_logger, 'Loss')[-1]:
        ...
        pred = outputs[1][-1]  # add_mean
        metric_logger.update(ssim=reduce_mean(torch.mean(g_ssim(pred / 255.0, gt))))
        metric_logger.update(psnr=reduce_mean(
            psnr_v2(pred, gt * 255.0, 4, 255.0)))  # reduce_mean(psnr_v2(add_mean(outputs), gt, 4, 255.0))
        metric_logger.update(grad_norm=grad_norm)

    # gather the stats from all processes
    # metric_logger.synchronize_between_processes()
    metric_logger.update(MSE=criterion.losses.log[-1][0])
...
def main(args):
    ...
    if args.eval:
        ...
        if args.lr_scheduler:
            ...
            scheduler.lr_scheduler = torch.optim.lr_scheduler.StepLR(
