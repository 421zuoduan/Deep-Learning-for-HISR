...
def convert_image_np(inp, denorm=False):
...
def visualize_stn(axes, data, pred, pan):
    with torch.no_grad():
        # Get a batch of training data
        input_tensor = pp.showimage8(data)  # data.cpu()
        transformed_input_tensor = pp.showimage8(pred)  # pred.cpu()

        # input_tensor = torch.from_numpy(input_tensor).permute(1, 2, 0)
        # transformed_input_tensor = torch.from_numpy(transformed_input_tensor).permute(0, 3, 1, 2)
        #
        ...
        axes[1].imshow(transformed_input_tensor)
        axes[1].set_title('Transformed Images')

        axes[2].imshow(pan[0, 0, ...].cpu().numpy(), cmap='Greys_r')
        axes[2].set_title('pan image')
