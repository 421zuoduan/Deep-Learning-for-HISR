...
class VxmDense(nn.Module):
    ...
    def forward(self, source, target, registration=False):
        ...
        y = self.unet_model(x)# 64 16 64 64

        if hasattr(self, 'flow'):
            # Conv: transform into flow field
            flow_field = self.flow(y) #64 2 16 16

            # resize flow for integration
            pos_flow = flow_field
            if self.resize:
                pos_flow = self.resize(pos_flow)# 64 2 32 32
            ...
            if not registration:
                warped = self.transformer(source, y)
                return warped
            return y
