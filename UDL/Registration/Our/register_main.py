        ...
        for loss_dict in self.losses.items():
            ...
            if len(outputs) > 1:
                ...
                # losses.update(self.get_loss(loss, outputs, targets))
                    self.loss_dicts.update({k: loss(outputs[1], targets[1].mean(dim=1, keepdims=True))})
            else:
                self.loss_dicts.update({k: loss(outputs[0], targets[0])})
...
class EpochRunner():
    ...
    def __init__(self, args, sess):
        self.args = args
        out_dir, model_save_dir, tfb_dir = create_logger(args)
    ...
    def validate_framework(self, val_loader, model, criterion, epoch=0):
        ...
        for batch in metric_logger.log_every(val_loader, 1, header):
            ...
            outputs, r_outputs, flow = model(pan, lms, registration=True)

            # qv.FlowField.flow([pan, r_outputs], [flow[0, ...]], mode='grid')
            # qv.FlowField.flow(None, [flow[0, ...]], mode='grid')
            # plt.pause(1)
...
if __name__ == "__main__":
    ...
    model_path = './results/wv3/Vmx/Unet/model_2021-05-20-11-57/1516.pth.tar'
