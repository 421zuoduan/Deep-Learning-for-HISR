...
class Swinnet(PatchMergeModule):
    def __init__(self, args):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(self.embed_dim*2, self.in_channels, 3, 1, 1), nn.LeakyReLU(0.2, True))
        self.t = Bottleneck(img_size=self.img_size, patch_size=1, in_chans=9, embed_dim=self.embed_dim, num_heads=[8, 8, 8], window_size=4)
        self.visual_corresponding_name = {}
        init_weights(self.conv, self.conv1)
        init_w(self.t)
        self.visual_corresponding_name['gt'] = 'result'
        self.visual_names = ['gt', 'result']

    def forward(self, gt, pan, lms):
        '''
        :param pan:
        :param ms:
        :return:
        ...
        self.pan = pan
        self.lms = lms
        xt = torch.cat((self.lms, self.pan), 1)  # Bx34X64x64
        ...
        w_out = self.t(xt)
        self.result = self.conv1(w_out) + self.lms

        # # padding
        # self.result =self.result[:, :, 4:1004, 4:1004]
        return self.result
