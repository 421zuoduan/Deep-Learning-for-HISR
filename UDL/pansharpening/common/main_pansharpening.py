...
import numpy as np
# from option_fusionnet import args
# from FusionNet.fusionnet_main import build_fusionnet as build
# from PanNet.pannet_main import build_pannet as build
# from option_pannet import args
# from option_DCFNet import args
# from DCFNet.model_fcc_dense_head import build_DCFNet as build
...
class EpochRunner():
    ...
    def __init__(self, args, sess):
        self.args = args
        if args.use_log:
            _, out_dir, model_save_dir, tfb_dir = create_logger(args, args.experimental_desc, dist_print=args.global_rank)
    ...
    def eval_framework(self, eval_loader, model, criterion):
        ...
        # switch to evaluate mode
        model.eval()
        if self.args.dataset == 'wv3':
            sr = torch.zeros((20, 8, 256, 256))
        ...
        if args.dataset == 'wv3':
            save_name = saved_path + '/' + args.name + 'wv3_256.mat'
        elif args.dataset == 'harvard_x4':
            save_name = saved_path + '/' + args.name + 'harvard200.mat'
...
if __name__ == "__main__":
    # FIXME 引发日志错误
    from UDL.pansharpening.DCFNet.option_DCFNet import args
    from UDL.pansharpening.DCFNet.model_fcc_dense_head import build_DCFNet
    ...
    main(args)
    '''
    - using l2_regularization for nn.Conv2D
    - Averaged stats: lr: 0.000300  grad_norm: 0.027350  Loss: 0.0092 (0.0400301)   - Averaged stats: lr: 0.000300  grad_norm: 0.027350  Loss: 0.0062 (0.0370622)
    - Averaged stats: lr: 0.000300  grad_norm: 0.008475  Loss: 0.0056 (0.0074309)   - Averaged stats: lr: 0.000300  grad_norm: 0.008475  Loss: 0.0027 (0.0044821)
    5                                                                               - Averaged stats: lr: 0.000300  grad_norm: 0.004609  Loss: 0.0018 (0.0017295)
    - Epoch: [6] Total time: 0:00:09 (0.0338 s / it)
    - Averaged stats: lr: 0.000300  grad_norm: 0.003840  Loss: 0.0045 (0.0044680)   - Averaged stats: lr: 0.000300  grad_norm: 0.003840  Loss: 0.0016 (0.0015402)
    10                                                                              - Averaged stats: lr: 0.000300  grad_norm: 0.003714  Loss: 0.0010 (0.0011606)
    400 - Averaged stats: lr: 0.000300  grad_norm: 0.000861  Loss: 0.0005 (0.0003616)  reg_loss: 0.0080 (0.0078539)
    -  * Best validation psnr so far@1 0.0003612 in epoch 393
    '''