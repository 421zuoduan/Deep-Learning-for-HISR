...
def mpl_save_fig(filename):
    plt.savefig(f"{filename}.svg", format='svg', dpi=300, pad_inches=0, bbox_inches='tight')
...
def eval_framework(args, eval_loader, model):
    ...
    for batch, idx in metric_logger.log_every(eval_loader, print_seq, header):
        ...
        our_CC, our_PSNR, our_SSIM, our_SAM, our_ERGAS = analysis_accu(gt.cuda().squeeze(0), result_our, 4)
        # [val, index_cut] = find(PanNet2_Q8~=0&PanNet2_ERGAS<=10&PanNet2_SAM<=10);
        if idx not in [220, 231, 236, 469, 766, 914]:
            metric_logger.update(SAM=our_SAM)
            metric_logger.update(ERGAS=our_ERGAS)
            metric_logger.update(SSIM=our_SSIM)
            metric_logger.update(CC=our_CC)
            metric_logger.update(PSNR=our_PSNR)
